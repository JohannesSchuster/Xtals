...
# ---- 1D Lorentzian model ----
def oneD_Lorentzian(x, amplitude, mean, gamma, offset):
    return offset + amplitude * (gamma**2 / ((x - mean)**2 + gamma**2))
...
# ---- Initial guess for Lorentzian parameters: amplitude, mean, gamma, offset ----
initial_guess_1d = [y_data.max(), (x_min_fit + x_max_fit) / 2, 1.0, y_data.min()]

# ---- Fit 1D Lorentzian ----
try:
    popt_1d, _ = curve_fit(oneD_Lorentzian, x_data, y_data, p0=initial_guess_1d)
    print("1D Lorentzian fit succeeded.")
except Exception as e:
    popt_1d = None
    print("1D Lorentzian fit failed:", e)
...
if popt_1d is not None:
    ...
    y_fit = oneD_Lorentzian(x_fit, *popt_1d)
    plt.plot(x_fit, y_fit, 'r-', label='1D Lorentzian fit')
    
    # Show fitting window as shaded area
    plt.axvspan(x_min_fit, x_max_fit, color='green', alpha=0.2, label='Fit window')

plt.title("1D Lorentzian Fit on Horizontal Profile")
plt.xlabel("Pixel")
...
if popt_1d is not None:
    with open('spot_lorentzian.txt', 'w') as f:
        f.write("1D Lorentzian fit parameters:\n")
        f.write(f"Amplitude: {popt_1d[0]:.6f}\n")
        f.write(f"Mean (center): {popt_1d[1]:.6f} px\n")
        f.write(f"Gamma (HWHM): {popt_1d[2]:.6f} px\n")
        f.write(f"Offset (baseline): {popt_1d[3]:.6f}\n")
    print("Parameters saved to spot_lorentzian.txt")
else:
    print("1D Lorentzian fit failed; no parameters saved.")
