import numpy as np
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit
from tifffile import imread

# ---- 1D Gaussian model ----
def oneD_Gaussian(x, amplitude, mean, sigma, offset):
    return offset + amplitude * np.exp(-((x - mean) ** 2) / (2 * sigma ** 2))

# ---- Load Image ----
file_path = r'C:\Users\ziegler\Desktop\Xtals\Data\X2025-06-19_22.14.45_Xlzso-NO3-3_r1_82_000.tif'
image_stack = imread(file_path)

image = image_stack[0]  # first 2D slice

# ---- FFT of the spot ----
fft_image = np.fft.fftshift(np.fft.fft2(image))
fft_magnitude = np.abs(fft_image)

# plt.figure(figsize=(6, 5))
# plt.imshow(np.log1p(fft_magnitude), cmap='gray')
# plt.title('FFT Magnitude (log scale)')
# plt.colorbar(label='log(FFT magnitude)')
# plt.show()

# ---- Auto find max intensity near center ----
center_x, center_y = image.shape[1] // 2, image.shape[0] // 2
radius = 2
x_min = max(center_x - radius, 0)
x_max = min(center_x + radius + 1, image.shape[1])
y_min = max(center_y - radius, 0)
y_max = min(center_y + radius + 1, image.shape[0])
search_region = image[y_min:y_max, x_min:x_max]
max_pos = np.unravel_index(np.argmax(search_region), search_region.shape)
max_y = y_min + max_pos[0]
max_x = x_min + max_pos[1]
print(f"Max intensity spot near center at (x={max_x}, y={max_y}), intensity={image[max_y, max_x]}")

# ---- Extract ROI ----
roi_size = 30
half_size = roi_size // 2
x1 = max(max_x - half_size, 0)
x2 = min(max_x + half_size, image.shape[1])
y1 = max(max_y - half_size, 0)
y2 = min(max_y + half_size, image.shape[0])
roi = image[y1:y2, x1:x2]

# ---- Extract horizontal profile (center row of ROI) ----
profile_y = roi.shape[0] // 2
x_data_full = np.arange(roi.shape[1])
y_data_full = roi[profile_y, :]

# ---- Automatically select fitting range around peak ----
peak_index = np.argmax(y_data_full)
window = 4  # pixels on each side

x_min_fit = max(peak_index - window, 0)
x_max_fit = min(peak_index + window, roi.shape[1] - 1)

mask = (x_data_full >= x_min_fit) & (x_data_full <= x_max_fit)
x_data = x_data_full[mask]
y_data = y_data_full[mask]

print(f"Fitting range pixels: {x_min_fit} to {x_max_fit}")
print("x_data:", x_data)
print("y_data:", y_data)

# ---- Initial guess for Gaussian parameters ----
initial_guess_1d = [
    y_data.max() - y_data.min(),      # amplitude (peak - baseline)
    (x_min_fit + x_max_fit) / 2,      # mean (center of fitting window)
    1.0,                             # sigma (spread)
    y_data.min()                     # offset (baseline)
]

# ---- Fit 1D Gaussian ----
try:
    popt_1d, _ = curve_fit(oneD_Gaussian, x_data, y_data, p0=initial_guess_1d)
except Exception as e:
    print("1D Gaussian fitting failed:", e)
    popt_1d = None

# ---- Plot horizontal profile and fitted curve ----
plt.figure(figsize=(8, 5))
plt.plot(x_data_full, y_data_full, 'b.', label='Data (horizontal profile)')
if popt_1d is not None:
    x_fit = np.linspace(x_min_fit, x_max_fit, 500)
    y_fit = oneD_Gaussian(x_fit, *popt_1d)
    plt.plot(x_fit, y_fit, 'r-', label='1D Gaussian fit')
plt.title("1D Gaussian Fit on Horizontal Profile")
plt.xlabel("Pixel")
plt.ylabel("Intensity")
plt.legend()
plt.show()

# ---- Save fitted Gaussian curve to file ----
if popt_1d is not None:
    # Define full path to existing Microcrystals folder on Desktop
    output_file = r'C:\Users\ziegler\Desktop\Microcrystals\spotspot.txt'

    # Save x_fit and y_fit as two columns
    with open(output_file, 'w') as f:
        f.write("# x_fit\t y_fit (Gaussian curve)\n")
        for x_val, y_val in zip(x_fit, y_fit):
            f.write(f"{x_val:.4f}\t{y_val:.4f}\n")

    print(f"Fitted curve saved to: {output_file}")

# ---- Print fitted parameters ----
if popt_1d is not None:
    print("\n1D Gaussian fit parameters:")
    print(f"  Amplitude: {popt_1d[0]:.2f}")
    print(f"  Mean: {popt_1d[1]:.2f} px")
    print(f"  Sigma: {popt_1d[2]:.2f} px")
    print(f"  Offset: {popt_1d[3]:.2f}")
else:
    print("\n1D Gaussian fit failed.")
